...
def main_test(args):
    ...
    if os.path.exists(save_path + r"/Patch_result") is False:
        os.makedirs(save_path + r"/Patch_result")
    ...
    with torch.no_grad(), open(save_path + '/predictions.txt', 'w') as file:
        for images, targets in metric_logger.log_every(test_loader, 1, header):
            ...
            for i in range(len(images)):
                ...
                if match:
                    ...
                    mat_file_path = os.path.join(save_path, f'Patch_result/patch_{row}_{col}_pred.mat')
        ...
        file.write(
            f"ConfusionMatrix_{genotype}:\n                       Pred label\n                      {genotype}0    {genotype}1\n")
        file.write(
            f"        True    {genotype}0  {conf_matrix[0, 0]:.0f},   {conf_matrix[0, 1]:.0f}\n")
        file.write(
            f"        label   {genotype}1  {conf_matrix[1, 0]:.0f},   {conf_matrix[1, 1]:.0f}\n\n")

        file.write(f"{genotype} Accuracy: {IDH_Acc * 100:.5f}%\n")
        file.write(f"{genotype} Sensitivity: {IDH_Sen * 100:.5f}%\n")
        file.write(f"{genotype} Specificity: {IDH_Spe * 100:.5f}%\n")
        file.write(f"{genotype} Recall: {IDH_Recall * 100:.5f}%\n")
        file.write(f"{genotype} Precision: {IDH_Pre * 100:.5f}%\n")
        file.write(f"{genotype} F1 Score: {IDH_F1 * 100:.5f}%\n")
        file.write(f"Whole Dice: {dice.value.item() * 100:.5f}%\n")

    # 绘制混淆矩阵
    plot_confusion_matrix(conf_matrix, labels_names=[f'{genotype}0', f'{genotype}1'],
                          save_path=save_path + '/Patch_result/confusion_matrix.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--genotype', type=str, default=r"MGMT")  # 动态赋值
